...
final readonly class AuthorizationCodeStore {
	...
	public function mint( AuthorizationGrant $grant ): string {
		...
		set_transient( 'counterhand_authorization_code_' . hash( 'sha256', $code ), $grant->to_array(), self::TTL_SECONDS );

		return $code;
	}

	public function consume( string $code ): ?AuthorizationGrant {
		$transient_key = 'counterhand_authorization_code_' . hash( 'sha256', $code );
